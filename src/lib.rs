...
use gl::types::*;
use std::{ffi::CString, mem, ops, ptr, str, thread, time, time::SystemTime};

// Settings
pub const STARTING_SCREEN_HEIGHT: i32 = 768;
pub const STARTING_SCREEN_WIDTH: i32 = 768;
pub const BUFFER_DATA_LENGTH: i32 = 5;
pub const SCREEN_MULTIPLIER: i32 = 1;
pub const SIZE_OF_FLOAT: i32 = 4;
pub const INF: f32 = 99999999.0;
pub const EPSILON: f32 = 0.001;
pub const REFLECTION_RECURSION_DEPTH: u8 = 0;
pub const NUM_THREADS: u8 = 6; // Needs to be even or one
pub const SHOW_DEBUG_LOGS: bool = true;
pub const SUBSAMP_RATE: u8 = 1; // 1 disables subsampling
pub const SUPERSAMP_RATE: u8 = 2; // 1 disables supersampling, max 15 due to u8 * u8 overflow
pub const SHADOWS_ON: bool = true;
pub const RENDER_STATIC_IMAGE: bool = false; // Disables re-rendering scene
pub const START_FULLSCREEN: bool = false;
pub const START_MAXIMIZED: bool = false;
pub const MAX_FPS: u8 = 60;
pub const MAX_FRAME_TIME_SEC: f32 = 1.0 / MAX_FPS as f32;

// Viewport Settings
pub const DISTANCE_TO_VIEWPORT: f32 = 1.0;

// Camera Settings
pub const CAMERA_STARTING_POSITION: Vec3 = Vec3 {
    a: 0.0,
    b: 1.0,
    c: 0.0,
};
pub const CAMERA_STARTING_ROTATION_X_ANGLE: f32 = 0.0;
pub const CAMERA_STARTING_ROTATION_Y_ANGLE: f32 = 0.0;
pub const CAMERA_STARTING_ROTATION_Z_ANGLE: f32 = 0.5;
pub const CAMERA_ROTATION_SPEED: f32 = 5.0;
...
pub fn build_rotation_matrix_3d(alpha: f32, beta: f32, gamma: f32) -> RotationMatrix3D {
    RotationMatrix3D {
...
pub fn setup_graphics(program: u32) -> (u32, u32) {
    let mut vao = 0;
    let mut vbo = 0;
    unsafe {
...
pub fn put_pixel(x: f32, y: f32, color: (u8, u8, u8), vertex_data: &mut Vec<GLfloat>) {
...
pub fn canvas_to_viewport(cx: f32, cy: f32, screen_width: i32, screen_height: i32) -> Vec3 {
    Vec3 {
...
pub fn vector_length(a: Vec3) -> f32 {
    let mut sum = 0.0;
    sum += a.a * a.a;
    sum += a.b * a.b;
    sum += a.c * a.c;
...
pub fn closest_intersection_shadow(
    camera_pos: Vec3,
    ray_direction: Vec3,
    t_min: f32,
    t_max: f32,
...
pub fn trace_ray(
    ...
    if let Some(cs) = closest_sphere {
        ...
        let l = compute_lighting(p, n, -1.0 * ray_direction, spheres, lights, cs.specular);
...
pub fn build_scene_objects() -> Vec<Sphere> {
    return vec![
        Sphere {
            center: Vec3 {
                a: 0.0,
...
pub fn build_scene_lights() -> Vec<Light> {
    return vec![
        Light {
...
pub fn send_camera_information_update(
...
pub fn start_scene_render_thread(
